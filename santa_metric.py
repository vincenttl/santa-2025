...
pack trees tightly. Running the file directly will save two figures and a text
summary into a timestamped directory under ``figures`` and print the scores for
both approaches.
"""
...
def compare_layouts(num_trees=25, seed=0, output_dir=None):
    """Run both strategies and save their figures and scores.

    If ``output_dir`` is not provided, a timestamped directory under
    ``figures`` is created to keep multiple runs side by side.
    """

    base_dir = Path("figures")
    if output_dir is None:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
        output_dir = base_dir / f"run-{timestamp}"
    else:
        provided = Path(output_dir)
        output_dir = provided if provided.is_absolute() else base_dir / provided
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare tree layout strategies and save artifacts.")
    parser.add_argument("--num-trees", type=int, default=25, help="Number of trees to place (default: 25)")
    parser.add_argument("--seed", type=int, default=0, help="Random seed for reproducibility")
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Directory to store the run outputs (relative to ./figures unless absolute)",
    )

    args = parser.parse_args()

    before, after, summary_file = compare_layouts(args.num_trees, args.seed, args.output_dir)
