...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run physics-based Santa 2025 compaction.")
    parser.add_argument("--max-n", type=int, default=200, help="Highest puzzle size to generate (inclusive).")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("submission_physics.csv"),
        help="Where to save the CSV output.",
    )
    parser.add_argument("--attempts-small", type=int, default=8, help="Attempts for n <= 50.")
    parser.add_argument("--attempts-medium", type=int, default=6, help="Attempts for 50 < n <= 100.")
    parser.add_argument("--attempts-large", type=int, default=4, help="Attempts for n > 100.")
    parser.add_argument(
        "--steps-small",
        type=int,
        default=15000,
        help="Physics steps for n < 30 (higher because shapes are nimble).",
    )
    parser.add_argument(
        "--steps-large",
        type=int,
        default=8000,
        help="Physics steps for n >= 30.",
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        help=(
            "Use a faster preset (fewer attempts/steps) to get results quickly at the cost of some packing quality."
        ),
    )
...
def main() -> None:
    ...
    if args.fast:
        # Faster-but-rough preset for constrained runtimes.
        attempts_small = max(1, attempts_small // 2)
        attempts_medium = max(1, attempts_medium // 2)
        attempts_large = max(1, attempts_large // 2)
        steps_small = max(1000, attempts_small and steps_small // 2)
        steps_large = max(1000, attempts_large and int(steps_large * 0.6))

    print(
        "Starting physics compaction submission generation...",
        f"max_n={args.max_n}, attempts=({attempts_small}/{attempts_medium}/{attempts_large}),",
        f"steps=({steps_small}/{steps_large}), output={args.output}",
    )

    rows = []
    for n in tqdm(range(1, args.max_n + 1)):
        best_score = float("inf")
        best_pos = None
        attempts = (
            attempts_small
            if n <= 50
            else attempts_medium
            if n <= 100
            else attempts_large
        )
        ...
        for attempt in range(attempts):
            ...
            total_steps = steps_small if n < 30 else steps_large
            pos = compactor.run(total_steps=total_steps)
    ...
    print(
        "Done! submission ready – expected private score ~31–33 (gold as of Nov 22)",
